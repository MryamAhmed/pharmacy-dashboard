...
- `data/models/login_data_model.dart` / `data/models/login_response_model.dart` — data-layer login payload and response envelope. `LoginData.toDomain()` maps the backend token/user payload into `AuthEntity`, and `LoginResponse.toDomain()` delegates to that payload mapper so repositories can convert successful login responses into domain entities.
